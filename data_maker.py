...
# ultralytics_path="C:/project_file/ultralytics_v11/"
model = YOLO(f'{ultralytics_path}runs/detect/sp_holes_{model_ver}/weights/best.pt')
# model = YOLO(f'{ultralytics_path}runs/detect/sp_holes_{model_ver}/weights/best.pt')
...
def main():
    ...
    # img_dir = "C:/project_file/AI_Supervise_processing/saved_img/old_data"
    img_dir = "C:/project_file/AI_Supervise_processing/saved_img/exp"
    # img_dir = "./unmake_data"
    ...
    for img_name in os.listdir(img_dir):
        if img_name.endswith(('.png', '.jpg', '.jpeg')):  # 只处理图片文件
            ...
            # rate=1920
            # frame = cv2.resize(frame,(rate,int(width*(rate/height))),interpolation=cv2.INTER_AREA)
            ...
            for result in last_results:
                ...
                for box in boxes:
                    ...
                    # if random_number==1:
                    #     cropped_img = frame[int(y1):int(y2)+by, int(x1):int(x2)+bx]
                    # elif random_number==2:
                    #     cropped_img = frame[int(y1)-by:int(y2), int(x1)-bx:int(x2)]                        
                    # elif random_number==3:
                    #     cropped_img = frame[int(y1):int(y2)+by, int(x1)-bx:int(x2)]                        
                    # else:
                    #     cropped_img = frame[int(y1)-by:int(y2), int(x1):int(x2)+bx]                        
                    ...
                    if cropped_img is not None and cropped_img.size > 0:  # 检查是否裁剪成功
                        ...
                        cv2.imwrite(f"./data/{base_name}_{count}_scale{r}.png", resized_img)
